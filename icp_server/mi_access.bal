...
isolated function miWrite(types:Runtime runtime, string method, string path, json body,
        string userId, string? requestId) returns MIAnswer|error {
    if !miTunnelEnabled {
        ...
        auditMIWrite(runtime.runtimeId, method, path, body, userId);
    ...
    if queued is () {
        boolean created = check enqueueMIMutation(runtime, method, path, body, userId, operationId);
...
# Whether a queued write is past hope: the sweep has said so, or its deadline has passed.
#
...
isolated function operationGaveUp(types:CacheOperation row, int now) returns boolean =>
    row.status == types:CACHE_OP_EXPIRED || row.deadline <= now;
...
#
# Plain text stays a JSON string, which is what the tunnel does with it too. An empty body
# becomes `()`, not an error: a management API that answers 200 with nothing has answered.
