...
# What the agent is asked to execute. `identity` carries the ICP user so the agent can name
# them in its audit log; it is not what authorizes the call.
...
isolated function enqueueMIMutation(types:Runtime runtime, string method, string path,
        json body, string userId, string operationId) returns boolean|error {
    ...
    return storage:enqueueCacheOperation({
        ...
        data: miRequestDocument(method, path, body, userId)
    });
}
...
isolated function queryParamOf(string path, string name) returns string {
    ...
    foreach string pair in re `&`.split(path.substring(queryStart + 1)) {
        ...
        if eq is int && pair.substring(0, eq) == name {
            string|error decoded = url:decode(pair.substring(eq + 1), "UTF-8");
            return decoded is string ? decoded : pair.substring(eq + 1);
        }
    }
    return "";
}
...
isolated function lastPathSegment(string path) returns string {
    ...
    return lastSlash is int ? withoutQuery.substring(lastSlash + 1) : withoutQuery;
}
...
isolated function auditMIOperation(types:CacheOperation row, types:WorkflowCommandResult result) {
    ...
    auditMIWrite(row.target, method, path, body, actor);
}
